...
function App() {
  ...
  return <div className="cloud-app">
    <aside className="cloud-sidebar">
      <a href="index.html" className="cloud-brand"><img src="assets/logo-light.svg" alt="ScolaPilot" /></a>
...
function NavButton({ active, icon, label, count, onClick }) { return <button className={`cloud-nav-button ${active ? 'active' : ''}`} onClick={onClick}><span>{icon}</span>{label}{count !== undefined && <b>{count}</b>}</button> }
